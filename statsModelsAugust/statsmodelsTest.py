...
#for state in ['MD']:
    ...
    temp = temp[-5:]
    temp.index = temp.index + pd.DateOffset(months=5)
    temp.to_csv('../statesSeptemberSubmission/' + state + '/FORECASTnegative_binomial.csv')
    ...
    temp = temp[-5:]
    temp.index = temp.index + pd.DateOffset(months=5)
    temp.to_csv('../statesSeptemberSubmission/' + state + '/FORECASTpoisson.csv')
    ...
    try:
        ...
        temp = temp[-5:]
        temp.index = temp.index + pd.DateOffset(months=5)
        temp.to_csv('../statesSeptemberSubmission/' + state + '/FORECASTzero_inflated_negative_binomial.csv')
    except:
        pass
    ...
    temp = temp[-5:]
    temp.index = temp.index + pd.DateOffset(months=5)
    temp.to_csv('../statesSeptemberSubmission/' + state + '/FORECASTzero_inflated_poisson.csv')
