...
for state in [state for state in set(wnv_data['state']) if state != 'DC']:
    noaa = pd.read_csv('statesMaySubmission/'+state + '/NOAA_data.csv')
    temporal = pd.read_csv('statesMaySubmission/'+state + '/temporalData.csv')
    ...
    final.to_csv('statesMaySubmission/'+state + '/noaa+temporal.csv')
