...
move_column = wnv_data_NJ.pop("count")
wnv_data_NJ.insert(0, "count", move_column)

testDataSize = 50

wnv_train = wnv_data_NJ.head(len(wnv_data_NJ) - 50)

target = 'count'
target_mean = wnv_train[target].mean()
target_stdev = wnv_train[target].std()

wnv_test = wnv_data_NJ.tail(50)
...
model = ShallowRegressionLSTM(num_sensors=len(wnv_data_NJ.axes[1]) - 1, hidden_units=num_hidden_units)
...
compare_df = pd.DataFrame(index = wnv_test.index, columns=['predicted', 'actual'])
