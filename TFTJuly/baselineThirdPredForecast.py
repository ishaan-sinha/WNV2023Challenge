...
#for state in [i for i in wnv_data['state'].unique()]:
    ...
    state_data = state_data[5:].astype(np.float32)

    #We will make 12 forecasts, as we have 8 months ahead only for the last 12 months
    ts = TimeSeries.from_series(state_data['7monthsAhead'].dropna())
    ...
    dfY = dfY[-7:]
    dfY.to_csv('../statesJulySubmission/'+state+'/FORECASTthirdPred.csv')
