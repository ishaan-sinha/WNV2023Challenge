...
#for state in ['CA']:
    ...
    state_data = pd.concat([state_data, mosquitoData], axis=1).astype(np.float32)
    state_data.drop(['year', 'month'], axis=1, inplace=True)
    rando_data = state_data[state_data['7monthsAhead'].notna()]

    #We will make 8 forecasts, as we have 7 months ahead for the rest of the data
    ts = TimeSeries.from_series(rando_data['7monthsAhead'])
    ...
    dfY = dfY[-7:]
    dfY.to_csv('../statesJulySubmission/'+state+'/FORECASTwithArbovirusLocalizedMinMax.csv')
    print(state)
