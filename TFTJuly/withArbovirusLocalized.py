...
def getData(state):
    ...
    state_data['real_month_cos'] = np.cos((state_data.index + np.timedelta64(N_FC, 'M')).month * 2 * np.pi / 12)
    state_data['real_month_sin'] = np.sin((state_data.index + np.timedelta64(N_FC, 'M')).month * 2 * np.pi / 12)

    state_data['7monthsAhead'] = state_data['count'].shift(-N_FC)
    state_data['5monthsAgo/1yearbeforePred'] = state_data['count'].shift(12-N_FC)
...
for state in ['CA']:
    ...
    state_data = state_data.fillna(-1)

    state_data = state_data[state_data['7monthsAhead'] != -1]
    ...
    #plt.show()
    plt.savefig('../statesJulySubmission/'+state+'/train+testwithArbovirusLocalized.png')
    plt.clf()
    ts_pred = transformer.inverse_transform(ts_tpred)
    ...
    #plt.show()
    plt.savefig('../statesJulySubmission/'+state+'/testwithArbovirusLocalized.png')
    ...
    dfY = dfY[-N_FC:]
    dfY.to_csv('../statesJulySubmission/'+state+'/withArbovirusLocalized.csv')



df_results_mae.to_csv('../modelResults/July/withArbovirusLocalizedTest.csv')
