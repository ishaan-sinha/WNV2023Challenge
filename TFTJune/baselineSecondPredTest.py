...
df_results_mae = pandas.DataFrame(columns=['state', 'baselineSecondPredTest'])
def getData(state):
...
#for state in ['CA']:
    ...
    df_results_mae = df_results_mae.append({'state': state, 'baselineSecondPredTest': mae(ts_test, ts_pred)}, ignore_index=True)
    #plt.show()
    plt.savefig('../statesJuneSubmission/'+state+'/train+testBaselinesecondPred.png')
    plt.clf()
    ts_pred = transformer.inverse_transform(ts_tpred)
    ...
    #plt.show()
    plt.savefig('../statesJuneSubmission/'+state+'/testBaselinesecondPred.png')
...
df_results_mae.to_csv('../modelResults/June/baselineSecondPredTest.csv')
