...
#for state in ['CA']:
    ...
    mosquitoData = pd.read_csv('../statesJuneSubmission/'+ state +'/MonthlyMosquitoData500miles.csv')
    ...
    state_data = state_data[state_data['8monthsAhead'].notna()]
    ...
    #plt.show()
    plt.savefig('../statesJuneSubmission/'+state+'/train+testwithArbovirusLocalizedMinMax.png')
    plt.clf()
    ts_pred = transformer.inverse_transform(ts_tpred)
    ...
    #plt.show()
    plt.savefig('../statesJuneSubmission/'+state+'/testwithArbovirusLocalizedMinMax.png')
    ...
    dfY.to_csv('../statesJuneSubmission/'+state+'/withArbovirusLocalizedMinMax.csv')
    dfY = dfY[-8:]
    dfY = dfY.reset_index()
    print(state)


df_results_mae.to_csv('../modelResults/June/withArbovirusLocalizedTestMinMax.csv')
